...
public class VentanaAEstrella extends JFrame {
	...
	*/
	...
	public void  contruyeMapaFijo()
	{
    	...
    	for(int i= 0 ; i < ri.nextInt(5); i++){
    		
    		mapa.introduceFinal(ri.nextInt(fila), ri.nextInt(columna));
    		
    		
    	}
    	for(int i= 0 ; i < Constantes.NUMERO_DE_INTRANSITABLES_ALEATORIOS; i++){
    		
    		mapa.introducePeligro(ri.nextInt(fila), ri.nextInt(columna));
    		
    	
    	}
    	
    	for(int i= 0 ; i < Constantes.NUMERO_DE_COSTES_ALEATORIOS; i++){
    		
    		mapa.introduceCoste(ri.nextInt(fila), ri.nextInt(columna), ri.nextDouble());
    		
    	
    	}
    	for(int i= 0 ; i < Constantes.NUMERO_DE_ALTURAS_ALEATORIOS; i++){
    		
    		mapa.introduceAltura(ri.nextInt(fila), ri.nextInt(columna), ri.nextInt(1000));
